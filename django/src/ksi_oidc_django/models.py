...
class KsiOidcClientConfig(SingletonModel):
    ...
    def save(self, **kwargs):
        if (self.configuration_endpoint is None) != (self.registration_token is None):
            raise ValueError("KsiOidcProviderConfig: Both configuration_endpoint and registration_token must be set or unset together")

        if (self.client_id is None) != (self.client_secret is None):
            raise ValueError("KsiOidcProviderConfig: Both client_id and client_secret must be set or unset together")

        if (self.configuration_endpoint is not None) and (self.client_id is None):
            raise ValueError("KsiOidcProviderConfig: client_id must be set when configuration_endpoint is set")
