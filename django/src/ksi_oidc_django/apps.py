...
class KsiOidcAppConfig(AppConfig):
    ...
    def verify_correct_setup():
        ...
        try:
            our_index = settings.MIDDLEWARE.index(
                "ksi_oidc_django.middleware.OidcAuthMiddleware"
            )
        except ValueError:
            raise ImproperlyConfigured(
                "OidcAuthMiddleware is not installed! It is required for the refreshing and expiration of sessions"
            )

        try:
            django_auth_index = settings.MIDDLEWARE.index(
                "django.contrib.auth.middleware.AuthenticationMiddleware"
            )
        except ValueError:
            logger.warning(
                "Django AuthenticationMiddleware is not installed! It is required for authentication"
            )
            return  # The index check is skipped in this case

        if our_index != django_auth_index + 1:
            logger.warning(
                "OidcAuthMiddleware should be placed directly after Django's AuthenticationMiddleware in the MIDDLEWARE list"
            )
