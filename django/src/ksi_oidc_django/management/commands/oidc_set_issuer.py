...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if config.issuer is not None:
            self.stdout.write(
                f"The OIDC issuer URI is currently set to: {config.issuer}"
            )
        ...
        if new_issuer == "":
            raise CommandError(
                "The issuer URI cannot be empty. Use the 'manage.py oidc_reset' command to reset all configuration."
            )

        try:
            OidcClient.load(issuer=new_issuer)
        except Exception as e:
            raise CommandError(
                f"Failed to load the OIDC Provider configuration from {new_issuer}/.well-known/openid-configuration: {e}"
            )
